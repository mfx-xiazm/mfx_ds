...
@interface ONPictureCropperView () <UIGestureRecognizerDelegate>
...
@implementation ONPictureCropperView
...
- (id)initWithCropImage:(UIImage*)cropImage cropSize:(CGSize)cropSize {
	if (self == [super init]) {
		...
		_cropRect = CGRectMake(0, (kScreenHeight - cropSize.height)/2, cropSize.width, cropSize.height);
		...
		_topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, _cropRect.origin.y)];
		_topView.backgroundColor = [UIColor blackColor];
		_topView.alpha = 0.7;
		_topView.userInteractionEnabled = NO;
		[self addSubview:_topView];
		
		_bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, _cropRect.origin.y + _cropRect.size.height, kScreenWidth, (kScreenHeight - _cropRect.origin.y - _cropRect.size.height))];
...
- (void)DoubleTapAction:(UIGestureRecognizer *)sender {
	...
	if (self.imgView.transform.a > 1 &&  self.imgView.transform.d > 1) {
		[UIView animateWithDuration:0.2f animations:^(void) {
			...
			self.imgView.center = self.cropperView.center;
		} ];
	} else {
		[UIView animateWithDuration:0.2f animations:^(void) {
...
- (void)actionRotate {
	[UIView animateWithDuration:0.15 animations:^{
		...
		if (self.imgView.frame.size.width < self.cropperView.frame.size.width || self.imgView.frame.size.height < self.cropperView.frame.size.height) {
			double scale = MAX(self.cropperView.frame.size.width/self.imgView.frame.size.width, self.cropperView.frame.size.height/self.imgView.frame.size.height) + 0.01;
			self.imgView.transform = CGAffineTransformScale(self.imgView.transform, scale, scale);
		}
	}];
}
